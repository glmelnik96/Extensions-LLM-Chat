...
| Doc | Purpose |
|-----|---------|
| [final-architecture.md](final-architecture.md) | Implemented architecture (config, pipeline, KB, prompt-library, diagnostics). |
| [final-target-architecture.md](final-target-architecture.md) | Target design reference (multi-pass, KB, session schema). |
| [pipeline-runtime-flow.md](pipeline-runtime-flow.md) | Pipeline stages, model roles, rules, status line. |
| [runtime-state-schema.md](runtime-state-schema.md) | Session and pipeline state schema. |
...
| [host-bridge-notes.md](host-bridge-notes.md) | Panel ↔ After Effects (target refresh, Apply). |
