...
**Change journal (2026-09-03)** — still 70 tools. `lib/pure/changeJournal.js` keeps what the agent itself changed in the comp (mutating calls one line each with layer ids, recipe parameters, key ranges, expressions; layers it created; diff summary) and the loop inserts it as a `[SYSTEM] JOURNAL` message before every later request, so a follow-up edits the existing rig in place. Persisted with the chat session; the eval runner builds the same entries between turns (`--journal on|off`). New two-turn case `orbit-then-faster`. Honest result: the "ускорь rebuilds the rig" hypothesis did not reproduce even without the journal (the model's own report named the null); the journal made the follow-up leaner (3 calls vs 4, −17% tokens) and guards multi-turn edits. The regression also exposed that a plan glued to `[[final]]` on the plan turn ended the run with zero tools (the marker short-circuit bypassed the phantom-done guard; two of yesterday's four "plan restated" failures were this path) — the marker path now runs the same guard and continues into tool turns; and `slide_in` on a parented layer now says its keys live in PARENT space (the model had deleted the recipe's correct keys and rewritten them from comp numbers); `set_property_value` now refuses an animated property (`PROPERTY_HAS_KEYFRAMES`, `replace_keyframes:true` for the explicit case) after the model pinned Opacity = 100 over a correct stagger and the host silently dropped the keys, and a reply that claims completed work after a no-change VERIFY gets one nudge; the ramp note now ignores fades shorter than 0.4 s (it had talked the model out of a correct 0.2-s fade five times in a row) and the corpus `appearanceTime` check measures the first VISIBLE key instead of the first key (fade-in, already-staggered 2/2, stagger-new 2/2 and popin-late all PASS (6/6 case runs) after the check fix and the softened note) — live in AE: the refusal keeps both keys and names the range 0.4–0.6s, replace_keyframes:true removes them and sets 100; the silent-deletion path did not recur in 4 later stagger-new runs; slide note: live message verified in AE (local keys [440,-240] vs comp landing [1400,300]); slide-from-right 3/3 PASS after the note. Confirmation: the one-call / zero-tool signature did not recur in 8 later stagger-new runs, 4 shrink runs and 4 popin-late runs; shrink now takes 2 calls (plan + nudge + set_property_value, ~73k tokens). Full regression: 22/26 on the first full run (3.2M tokens, 790 s) — three zero-tool marker-path cases (shrink, stagger-new, popin-late) plus slide-from-right; explicit-mapping and question-only fixed, orbit-then-faster PASS; every failed case green on focused re-runs after the guards described next (shrink 4/4, popin-late 4/4, slide-from-right 3/3). 292 tests pass.
