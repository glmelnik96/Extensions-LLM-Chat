...
;(function () {
  ...
  function buildCorrectionPrompt (issues) {
    ...
    lines.push('Data being present does NOT refute a visibility report: expressions and keyframes can be intact while the element is still invisible on screen. Before dismissing a "not visible" issue as a false positive, verify VISIBILITY itself — contrast against the background color, element size relative to the comp, on-screen position (including parent-space offsets), and the layer video switch (enabled). Only dismiss the issue if the element would genuinely be visible in the frame.')
