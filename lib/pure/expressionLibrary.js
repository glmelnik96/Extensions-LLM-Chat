...
(function () {
   ...
   * extraSnippets (optional): the user's personal snippets — searched
   * alongside the curated set and marked source:'user' in results.
   */
  function search (query, maxResults, extraSnippets) {
    ...
    var all = SNIPPETS
    if (extraSnippets && extraSnippets.length) all = SNIPPETS.concat(extraSnippets)
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      for (var t = 0; t < tokens.length; t++) {
        ...
        if (String(s.notes || '').toLowerCase().indexOf(tok) !== -1) score += 1
      }
      if (score > 0) scored.push({ score: score, snippet: s, user: i >= SNIPPETS.length })
    }
    ...
    var msg
    if (out.length > 0) {
      msg = 'Found ' + out.length + ' snippet(s) for "' + q + '". Apply with apply_expression; create any `requires` effects first via add_effect. NOTE: on localized AE (RU etc.) effect display names differ — reference effects by the exact name reported after add_effect, not the English name from the snippet.'
      if (userCount > 0) msg += ' ' + userCount + ' result(s) are from the user\'s personal library (source:"user") — prefer them when they match the request.'
    } else {
      msg = 'No library snippets match "' + q + '". Write the expression manually following the Expression Expertise rules.'
    }
    return { ok: true, message: msg, snippets: out }
  }
