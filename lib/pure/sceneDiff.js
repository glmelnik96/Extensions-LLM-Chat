...
;(function (root, factory) {
  ...
  function keyRangeText (k, layer, prop) {
    var t = k.numKeys + ' key' + (k.numKeys === 1 ? '' : 's') + ', ' + fmtTime(k.from) + '–' + fmtTime(k.to) + 's'
    // Before the first key AE holds the first key's value. For visibility
    // properties say so when that value is visible and the key sits after the
    // layer's in-point — the classic "visible from 1 s" window that shows
    // from t=0 (eval corpus 2026-09-03: 4 runs in a row).
    if ((prop === 'opacity' || prop === 'scale') && k.firstValue !== undefined && layer) {
      var fv = Array.isArray(k.firstValue) ? k.firstValue[0] : k.firstValue
      var inPt = typeof layer.inPoint === 'number' ? layer.inPoint : 0
      if (typeof fv === 'number' && fv > 0.5 && k.from > inPt + 0.04) {
        t += '; holds ' + fmtNum(k.firstValue) + ' BEFORE ' + fmtTime(k.from) + 's (visible from the in-point ' + fmtTime(inPt) + 's)'
      }
    }
    return t
  }
  ...
  function diffLayer (b, a) {
    ...
    for (k in animKeys) {
      ...
      if (!ba[k] && aa[k]) out.push(label + ': keyframes added (' + keyRangeText(aa[k], a, k) + ')')
        ...
        if (rangeChanged) out.push(label + ': keyframes changed (' + ba[k].numKeys + ' → ' + keyRangeText(aa[k], a, k) + ')')
        else if (ba[k].sig !== undefined && aa[k].sig !== undefined && ba[k].sig !== aa[k].sig) out.push(label + ': keyframe values edited (' + keyRangeText(aa[k], a, k) + ')')
      }
    }
    // Expressions.
    var be = exprMap(b)
