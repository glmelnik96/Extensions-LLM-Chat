...
(function () {
  ...
  var CORE_KNOWN_LIMITATIONS = [
    ...
    '- **A layer with its video switch OFF (`enabled: false` in the comp summary) renders NOTHING.** Animating it produces invisible work. Read `enabled` in the summary before building on a layer; if it is off, either enable it (`set_layer_switches({enabled:true})`) when it should be seen, or ask the user — and never report animation on a hidden layer as visible. `probe_motion` reports `visible:false` per sample for the same reasons (switch, in/out window, opacity 0, scale 0).',
