...
(function () {
  ...
  var CORE_WORKFLOW = [
    ...
    '   - **Standard motion patterns → `apply_motion_recipe`** (pop-in, slide-in from an edge, fade in/out, pulse, orbit, follow-with-delay, camera shake) — one call, keys from each layer\'s in-point, anchor/parent-space/off-screen math done in code. Hand-build keyframes only for motion that is not one of these.',
    '   - **Keyframes** for most other animation (position, scale, rotation, opacity). Use easing (bezier) for natural motion.',
  ...
  var CORE_KNOWN_LIMITATIONS = [
    ...
    '- **Hold keys act on the segment AFTER the key** (`out_type:"hold"`): an on/off visibility window needs out_type hold on every key of the window; `in_type:"hold"` + `out_type:"linear"` still ramps. Keyframe results warn when Opacity is half-transparent between keys.',
    ...
    '- **"Тряска камеры" / camera shake = `apply_motion_recipe(recipe:"shake")` with no layer_ids** — ONE call that builds a null rig every layer is parented to. Do NOT create a camera for it and do NOT switch layers to 3D to make a camera matter: that changes how every layer renders (perspective, sorting, motion blur) just to fake a shake. Only a comp that is ALREADY 3D with a camera gets its shake on that camera.',
