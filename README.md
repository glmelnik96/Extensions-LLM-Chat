...
- **Preset toolbar** — детерминированные пресеты Fade/Pop/Slide с параметрами
- **Сессии** — создание, переключение, переименование, очистка; метаданные в sidebar
- **Undo** — откат всех действий агента за один клик (batch-undo)
- **Stop** — отмена работающего агента
- **Export** — сохранение сессий в JSON на Desktop
- **Report** — LLM-анализ всех сессий → структурированный баг-репорт на Desktop
...
| `chatProvider.js` | Cloud.ru (SSE streaming) + Ollama API |
...
| `lib/ollamaVision.js` | Ollama vision (не подключён к агенту) |
...
- **Cloud.ru Foundation Models** — основной: chat/completions с tool calling и SSE streaming
- **Ollama** — локальный, путь существует в коде, но не выведен в UI модели
...
- Smoke-чеклист: [docs/qa-test-plan.md](docs/qa-test-plan.md)
- Полная матрица (35 тестов): [docs/manual-test-v3.md](docs/manual-test-v3.md)
